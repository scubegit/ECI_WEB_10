		...
		function generateProductList(customerId){
					...
					$('#prodList').append('<option value=' + 0+ '> Select Product  </option>');
...
$(document).on("click", "#AdHocReportSubmit", function(e){
			...
			if(ValidationForSelectBox("#reportErr2","Customer Name ",$('#custList')))
				...
				window.location.href = "QlAdHocReport?custId="+$("#custList").val()+"&prodId="+$("#prodList").val()+"&dateFrom="+$("#dateFrmad").val()+"&dateTo="+$("#dateToad").val();
...
function getlist(dataVal){
			...
			//	console.log("-- getAdHocReport=data.result==",data.result);--
				...
				'<a data-auto-download href="../ProApp/GeneratePDF/ATP'+test+'.pdf" class="table-input-btn cust-btn-style complete_job_cust_btn" download>Download ATP </a>'+
				...
				' data-target="#showdetails" id="showdetailsbtn" incid="'+data.id+'"  TaskId="'+data.TaskId+'">'+
