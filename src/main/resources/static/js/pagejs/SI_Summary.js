	...
	//				$.get(url+"getPMsummary/"+localStorage.getItem("userId"), function( data ) { //from API list
				...
				var dataVal = {
					...
					$.ajax({
						...
						type: 'POST',
						url: url+"getSIsummary",  //from API update data
						...
						console.log("----getTEList----click on CtrObj-------",CtrObj);
							...
							$.each( CtrObj, function( index, value ){
								...
								if(prev==curr){
									...
									$("#"+sidata).append('<div class="panel-heading"  data-toggle="collapse" data-parent="#accordion" href="#collapse'+index+'">'+
										//	'<h4 class="panel-title"> <a>'+value.Emp_Name+'</a></h4>
										    '</div><div id="collapse'+index+'" class="panel-collapse collapse">'+
											'<div class="panel-body"><div class="full-wd"><div class="wd33"><p>Assigned</p></div><div class="wd33"><p>WIP</p></div>'+
											'<div class="wd33"><p>Exit/Reject</p></div></div><div class="full-wd">'+
											'<div class="wd33"><p data-target="#unassModal"  data-toggle="modal" class="pointer_cursor unassModal1" sival="'+value.SI+'" regval="'+value.RegionId+'">'+value.Assigned+'</p></div>'+
											...
											'<div class="wd33"><p data-target="#taskStrtModal"  data-toggle="modal" class="pointer_cursor taskModal1" sival="'+value.SI+'" regval="'+value.RegionId+'">'+value.ExitReject+'</p></div></div></div></div></div></div>');
						...
						/*			alldiv = '<div class="col-md-4 " id='+value.RegionName+'> '+
									 			' <div class="block-hdr panel-heading"  data-toggle="collapse" data-parent="#accordion" href="#collapse'+index+'"  ><h2>'+value.RegionName+'</h2></div> '+
									// 			' <div class="block-cont-wrap"><div class="panel-group" id="accordion">'+
								 ...
								 //   '<h4 class="panel-title"><a>'+value.Emp_Name+'</a></h4> </div>
											    	' <div id="collapse'+index+'" class="panel-collapse collapse"> <div class="panel-body">'+
											    		' <div class="full-wd"><div class="wd33"><p>Assigned</p></div><div class="wd33"><p>WIP</p></div><div class="wd33"><p>Exit/Reject</p></div></div>'+
														' <div class="full-wd">'+
															'<div class="wd33"><p data-target="#unassModal"  data-toggle="modal" class="pointer_cursor unassModal1" sival="'+value.SI+'" regval="'+value.RegionId+'">'+value.Assigned+'</p></div>'+
															'<div class="wd33"><p data-target="#wipModal"  data-toggle="modal" class="pointer_cursor wipModal1" sival="'+value.SI+'" regval="'+value.RegionId+'">'+value.WIP+'</p></div>'+
															'<div class="wd33"><p data-target="#taskStrtModal"  data-toggle="modal" class="pointer_cursor taskModal1" sival="'+value.SI+'" regval="'+value.RegionId+'">'+value.ExitReject+'</p></div>'+
														'</div>'+
													'</div>'+
								//				'</div>'+
											'</div>';
							*/	  
									alldiv = '<div class="col-md-3 panel panel-default panel-heading"> <div class=" "  data-toggle="collapse" href="#collapse'+index+'"> '+
								 ...
								 //   '<h4 class="panel-title"><a>'+value.Emp_Name+'</a></h4> </div>
											    	' <div id="collapse'+index+'" class="panel-collapse collapse"> <div class="panel-body">'+
											    		' <div class="full-wd"><div class="wd33"><p>Assigned</p></div><div class="wd33"><p>WIP</p></div><div class="wd33"><p>Exit/Reject</p></div></div>'+
														' <div class="full-wd">'+
															'<div class="wd33">'+value.Assigned+'</div>'+
															'<div class="wd33">'+value.WIP+'</div>'+
															'<div class="wd33">'+value.ExitReject+'</div>'+
														'</div>'+
								//					'</div>'+
												'</div>'+
											'</div>';
