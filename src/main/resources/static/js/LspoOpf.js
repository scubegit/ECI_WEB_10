...
//		cache: false,
		success : function(res) {
			...
			console.log("-------CtrObj.LSPOresult-----------",CtrObj.LSPOresult);
			
			if (CtrObj.LSPOresult == "success"){
			
				
				if (CtrObj.LSPOItem[0].resultSoPay == "SoPaySuccess"){
					$.each( CtrObj.LSPOItem[0].dataSoPay, function( index, value ){
				...
				$("#lspoGSTNo").append('<option value="0">  --- Select GST Location---  </option>');
				if (CtrObj.LSPOItem[1].resultGstList == "gstSuccess"){
					$.each( CtrObj.LSPOItem[1].dataGstList, function( index, value ){
						
						$("#lspoGSTNo").append('<option value="'+value.GST_ID+'"> '+value.GST_No+' ('+ value.Location +') </option>');
					});
				}
	...
	//			$("#localVendor").append('<option value="0">  Select Vendor </option>');
				...
				if (CtrObj.LSPOItem[2].resultLocVen == "success"){
					$.each( CtrObj.LSPOItem[2].dataLocVen, function( index, value ){
						
						$("#localVendor").append('<option value="'+value.LcVendorID+'"> '+value.LcVendor_Name+' </option>');
					});
				}
...
$(document).on("click", "#openFPODoc", function(){

	$.ajax({
		url  : "setFPOInfo_invoice?"+new Date().getTime(),
		type : 'POST',
    	data : {
    		"orderID":getParameterByName('tempOrdNo')
	     },
		success : function(res) {
			...
			console.log("-------openFPODoc----------------",CtrObj);
			
			$('#FPOShipToAddress').empty();
			
			
			$("#FPOShipToAddress").append('<option value="0"> -- Select Ship To Address -- </option>');
			
			if (CtrObj.LSPOresult == "success"){
			
				if (CtrObj.LSPOData[0].resultLiplLoc == "liplLocSuccess"){
					$.each( CtrObj.LSPOData[0].dataLiplList, function( index, value ){
						
						$("#FPOShipToAddress").append('<option value="'+value.Lipl_ID+'"> '+value.Lipl_City+' </option>');
					});
				}
			}
		}
...
$(document).on("click", "#forLSPOinvoice", function(){
	...
	$("#LSPOInvoiceTbody").empty();
	 ...
	 if(ValidationForSelectBox("#ErrFpo","Local Vendor ",$("#localVendor")))
		 if(ValidationForSelectBox("#ErrFpo","GST Location ",$("#lspoGSTNo")))
 	...
 	if(NotAllowedNullVal("#ErrOPF","Shipment To",$("#shipmentTo")))
 		if(NotAllowedNullVal("#ErrOPF","Mode Of Dispatch",$("#modeOfDispatch")))
 			if(NotAllowedNullVal("#ErrOPF","Special Instructions",$("#specialInstructions")))
 				 if(ValidationForSelectBox("#ErrOPF","Freight ",$("#freight")))
 					 if(ValidationForSelectBox("#ErrOPF","Insurance ",$("#insurance"))){
			$.ajax({
...
$(document).on("click", "#forFPOinvoice", function(){
	...
	$("#ErrOPF").empty();
 	...
 	var data = { 
 			orderIDV				: orderID, 			
 			FPOShipToAddressV		: $.trim($("#FPOShipToAddress").val()),
 	...
 	if(ValidationForSelectBox("#ErrFpo","Ship To Address ",$("#FPOShipToAddress")))
...
$(document).on("click", "#serCIFinvoice", function(){
	...
	$.ajax({
		...
		success : function(res) {
			...
			$("#GSTNo").append('<option value="0"> -- select -- </option>');
			
			$.each( CtrObj.GstList.dataGstList, function( index, value ){
...
$(document).on("click", "#forSerCIFinvoice", function(){
   ...
   if(ValidationForSelectBox("#ErrSerCIF","GST Location  ",$("#GSTNo")))
    if(NotAllowedNullVal("#ErrSerCIF","Bill No ",$("#billNo")))
    	...
    	var data = { 
 			...
 			RemarksTaxInv	: $("#RemarksTaxInv").val()
...
function allLspoOpfList(){
	...
	$.ajax({
		...
		success : function(res) {
			...
			console.log("-------------LSPOFilePath CtrObj----------------",CtrObj);
			
			if (CtrObj.result == "success"){
				
				if( CtrObj.LspoData.length == 0 && CtrObj.OPFData.length == 0 && CtrObj.FpoData.length == 0 && CtrObj.CifData.length == 0){
				...
				$.each( CtrObj.CifData, function( index, value ){
					
					$("#CifList").append("<li><a href='downloadCIF?CIFFilePath="+value.cif_filePath+"'>"+ value.cif_fileName +"</a></li>");
				});
			}
		}
	});
...
function allSerList(){
	...
	$.ajax({
		url  : "getSerGeneratedPdfList_invoice?"+new Date().getTime(),
		type : 'POST',
    	data : {
    		"dataVal" : orderID
    	},
		success : function(res) {
			...
			if (CtrObj.result == "success"){
				...
				$.each( CtrObj.TaxInvData, function( index, value ){
					
					$("#TaxInvList").append("<li><a href='downloadSerCIF?SerCIFFilePath="+value.TaxInv_filePath+"'>"+  value.TaxInv_fileName +"</a></li>");
				});
			}
		}
	});
